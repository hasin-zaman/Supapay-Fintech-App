...
class _ChatState extends State<Chat> {
  ...
  Future<void> initializeChat() async {
    ...
    if (accountNumber!=null) {
      final fetchedMessages = await getChat(accountNumber);
                  ...
                  child: Card(
                    ...
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.only(
                        topLeft: Radius.circular(message.sentByUser ? 12 : 0),
                        topRight: Radius.circular(message.sentByUser ? 0 : 12),
                    ...
                    child: FloatingActionButton(
                      onPressed: () async {
                        if(messageController.text.length!=0){
                          final SharedPreferences prefs = await SharedPreferences.getInstance();
                          final accountNumber=await prefs.getString('accountNumber');
                          final UserModel? user=await getUser(accountNumber);

                          final message=MessageModel(
                              message: messageController.text,
                              date: DateTime.now(),
                              sentByUser: user?.role=="Customer" ? true : false);
                          ...
                          storeMessage(message, accountNumber!);
                          storeChat(chat);
